...
export default function ProductCard({ id, nombre, precio_final, cuotas_cantidad, cuotas_valor, garantia_meses, stock }) {
  const emoji = emojiPara(nombre, id);
  const { agregar, cantidadEnCarrito } = useCarrito();
  const [aviso, setAviso] = useState(false);

  const enCarrito = cantidadEnCarrito(id);
  const llegueAlMax = enCarrito >= stock;

  const handleAgregar = () => {
    const exito = agregar({ id, nombre, precio_final, stock }, 1);
    if (!exito) {
      setAviso(true);
  ...
  return (
    <div className="bg-white p-4 rounded-gourmet flex flex-col shadow-sm transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-[#3D2B1F]/10 group">
      ...
      <div className="p-6 text-center flex-1 flex flex-col">
        ...
        <p className="text-stone-300 text-xs mb-3">Stock: {stock} unidades</p>
        ...
        <div className="mt-auto">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-stone-50">
            ...
            <button
              ...
              title={llegueAlMax ? "Stock maximo alcanzado" : "Agregar al carrito"}
            >
